...
export async function compile(options: CompileOptions): Promise<CompileResult> {
  ...
  if (!fs.existsSync(path.join(workDir, texFile))) {
    return {
      ok: false,
      rawLog: '',
      errors: [{ type: 'missing_file', message: `File not found: ${path.join(workDir, texFile)}` }],
      warnings: [],
  ...
  return new Promise((resolve) => {
    ...
    const args =
      engine === 'tectonic'
        ? options.outputDir
          ? ['--outdir', options.outputDir, texFile]
          : [texFile]
        : [
            '-interaction=nonstopmode',
            '-halt-on-error',
            ...(options.outputDir ? [`-output-directory=${options.outputDir}`] : []),
            texFile,
          ];
    ...
    child.on('close', () => {
      ...
      if (timedOut) {
        resolve({
          ok: false,
          rawLog,
          errors: [{ type: 'unknown', message: `Compile timeout (${options.timeoutMs}ms)` }],
          warnings,
          durationMs: Date.now() - startTime,
          engine,
          timestamp: new Date().toISOString(),
    ...
    child.on('error', (err: any) => {
      ...
      resolve({
        ok: false,
        rawLog: '',
        errors: [{ type: 'unknown', message: msg }],
        warnings: [],
        durationMs: Date.now() - startTime,
        engine,
        timestamp: new Date().toISOString(),
