...
export function assembleFullPaper(paper: FullPaper, outputDir: string): string {
  ...
  const preamble = applyTemplateVariables(template.preamble, {
    title: escapeLatex(paper.title),
    authors: escapeLatex(authorsStr),
  });
  const sectionIncludes = paper.sections
    .map((s) => applyTemplateVariables(template.sectionIncludeTemplate, { texPath: s.texPath.replaceAll('\\', '/') }))
    .join('\n');
  const bibSection = paper.bibFilePath
    ? applyTemplateVariables(template.bibliographyTemplate, { bibPath: paper.bibFilePath })
    : '% No bibliography';
...
export function createLatexProject(config: {
  rootDir: string;
  title: string;
  authors: string[];
  templateId?: string;
  ...
  for (const dir of ['sections', 'figures', 'output', 'logs'])
    fs.mkdirSync(path.join(config.rootDir, dir), { recursive: true });
  const projConfig: ScholariumProjectConfig = {
...
export function writeSectionFile(
  ...
  const ref: SectionRef = {
    ...
    title,
    texPath: `sections/${fileName}`,
    status: 'drafting',
    version: 1,
  };
  const idx = project.sections.findIndex((s) => s.id === sectionId);
  if (idx >= 0) project.sections[idx] = ref;
  else project.sections.push(ref);
