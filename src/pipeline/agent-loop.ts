...
import type {
  ...
  AgentLoopAction, AgentLoopStep, AgentLoopSectionState,
  AgentLoopState, AgentLoopOptions,
} from '../types/index.ts';
...
export class AgentLoopOrchestrator {
  ...
  constructor(deps: AgentLoopDeps) {
    this.deps = deps;
    this.loopState = { paperId: '', steps: [], sectionStates: [], currentIteration: 0, maxIterations: 0, terminated: false };
  }
  ...
  async runLoop(
    ...
    while (this.loopState.currentIteration < maxIterations && !this.loopState.terminated) {
      ...
      if (taskId && taskManager) {
        taskManager.updateProgress(taskId, Math.round((this.loopState.currentIteration - 1) / maxIterations * 100), `${iterLabel} scheduling...`);
      }
      ...
      for (const { section: sectionDef } of priorityOrder) {
        ...
        if (action === 'write_section') {
          if (taskId && taskManager) {
            taskManager.updateProgress(taskId, Math.round((this.loopState.currentIteration - 1) / maxIterations * 100), `${iterLabel} writing ${sectionDef.title}...`);
          }
          try {
            const { section, state } = await orch.writeSection(paperId, outline, sectionDef, {
              mock, skipAntiAI, taskId, taskManager,
            });
            ...
            if (existingIdx >= 0) existingSections[existingIdx] = section; else existingSections.push(section);
            this.recordStep(action, sectionDef, state === 'passed' || state === 'human_review' ? 'success' : 'failure', `state=${state}`);
            anyActionTaken = true;
          } catch (e: any) {
          ...
          if (taskId && taskManager) {
            taskManager.updateProgress(taskId, Math.round((this.loopState.currentIteration - 1) / maxIterations * 100), `${iterLabel} auditing ${sectionDef.title}...`);
          }
          ...
          if (taskId && taskManager) {
            taskManager.updateProgress(taskId, Math.round((this.loopState.currentIteration - 1) / maxIterations * 100), `${iterLabel} revising ${sectionDef.title}...`);
          }
          try {
            const { section, state } = await orch.writeSection(paperId, outline, sectionDef, {
              mock, skipAntiAI, taskId, taskManager, maxRounds: 2,
            });
            secState.status = section.status;
            secState.version = section.version;
            const existingIdx = existingSections.findIndex(es => es.id === section.id);
          ...
          if (taskId && taskManager) {
            taskManager.updateProgress(taskId, Math.round((this.loopState.currentIteration - 1) / maxIterations * 100), `${iterLabel} Bible check...`);
          }
    ...
    const finalSections: Section[] = outline.sections.map(s => {
      ...
      return existing ?? {
        ...
        title: s.title, status: state.status, version: state.version,
      };
    });
  ...
  private recordStep(action: AgentLoopAction, sectionDef: OutlineSection, result: 'success' | 'failure' | 'skipped', detail?: string) {
    this.loopState.steps.push({
      action, sectionId: sectionDef.id, sectionTitle: sectionDef.title,
      round: this.loopState.currentIteration,
      timestamp: new Date().toISOString(), result, detail,
    });
  }
