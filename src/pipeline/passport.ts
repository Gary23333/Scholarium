  ...
  createPassport(paperId: string): MaterialPassport {
    ...
    const passport: MaterialPassport = {
      ...
      bible: bible.map((b: any) => ({
        id: b.id, paperId: b.paper_id, category: b.category, key: b.key, value: b.value,
        sourceSectionId: b.source_section_id, sourceType: b.source_type,
        sourceArtifactVersion: b.source_artifact_version, confidence: b.confidence,
        approvalStatus: b.approval_status, immutable: !!b.immutable,
      })),
      citations: citations.map((c: any) => ({
        id: c.id, paperId: c.paper_id, citeKey: c.cite_key, bibtex: c.bibtex,
        doi: c.doi, title: c.title, authors: c.authors, year: c.year,
        verified: !!c.verified, approvalStatus: c.approval_status ?? 'approved',
        source: c.source ?? 'user', matchConfidence: c.match_confidence ?? 1.0,
        lastVerifiedAt: c.last_verified_at, embedding: null,
