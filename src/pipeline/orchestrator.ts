...
import type {
  PaperOutline, OutlineSection, Section, SectionWriteState,
  PipelineRun, PipelinePhase, FixInstructions, BibleEntry,
} from '../types/index.ts';
...
export class PipelineOrchestrator {
  ...
  async runPlanning(paperId: string, confirmedFocus: any, journalProfile?: any, options?: { mock?: boolean }): Promise<PaperOutline> {
  ...
  async writeSection(paperId: string, outline: PaperOutline, sectionDef: OutlineSection, options: WriteSectionOptions = {}): Promise<{ section: Section; state: SectionWriteState }> {
    const { mock = false, maxRounds = 3, aiThreshold = 0.5, skipAntiAI = false, taskId, taskManager } = options;

    let section: Section = {
      id: sectionDef.id, paperId,
      sectionNumber: outline.sections.findIndex(s => s.id === sectionDef.id) + 1,
      title: sectionDef.title, status: 'pending', version: 1,
    };
    ...
    while (round < maxRounds) {
      ...
      // ① Composer
      if (taskId && taskManager) taskManager.updateProgress(taskId, 15 + round * 20, `正在组装上下文${roundLabel}...`, 'composer');
      const contextPackage = await this.deps.composer.execute({
        outline, section: sectionDef, bible: this.deps.bible, paperId,
        fixInstructions: pendingFixes, previousDraft: draft || undefined,
      }, { mock });
      if (taskId && taskManager) taskManager.updatePhase(taskId, 'composer', { status: 'completed', progress: 100 });

      // ② Writer
      if (taskId && taskManager) taskManager.updateProgress(taskId, 30 + round * 15, `正在撰写内容${roundLabel}...`, 'writer');
      const writerOutput = await this.deps.writer.execute({
        blueprint, context: contextPackage, previousDraft: draft || undefined,
      }, { mock });
      draft = writerOutput.texContent;
      if (taskId && taskManager) taskManager.updatePhase(taskId, 'writer', { status: 'completed', progress: 100 });

      // ③ Observer
      if (taskId && taskManager) taskManager.updateProgress(taskId, 45 + round * 10, '正在提取结构化内容...', 'observer');
      ...
      for (const entry of extraction.entries) {
        this.deps.bible.addEntry({
          paperId, category: entry.category, key: entry.key, value: entry.value,
          sourceSectionId: section.id, sourceType: 'agent', sourceArtifactVersion: section.version,
          confidence: entry.confidence, approvalStatus: 'approved', immutable: false,
        });
      }
      ...
      const normalizeResult = await this.deps.normalizer.execute({
        draft, targetWordCount: sectionDef.estimatedPages * 800,
        currentWordCount: writerOutput.wordCount, bibleEntries: this.deps.bible.getEntries(paperId),
      }, { mock });
      ...
      if (taskId && taskManager) taskManager.updateProgress(taskId, 62 + round * 5, '正在执行内容治理...', 'governance');
      const govCtx = buildGovernanceContext(
        this.deps.bible.getEntries(paperId),
        sectionDef,
        outline.sections.filter(s => outline.sections.indexOf(s) < outline.sections.indexOf(sectionDef)).map(s => ({ id: s.id, title: s.title, summary: s.coreArgument })),
      );
      ...
      if (!govResult.passed) {
        pendingFixes = {
          instruction: `Content governance found ${govResult.stats.critical} critical issues: ${govResult.violations.filter(v => v.severity === 'critical').map(v => v.message).join('; ')}`,
          round,
        ...
        section.status = 'needs_fix'; section.version++; round++;
        if (taskId && taskManager) taskManager.updateProgress(taskId, 62 + round * 5, `内容治理未通过，进入第${round + 1}轮重试`);
        continue;
      }
      if (taskId && taskManager) taskManager.updatePhase(taskId, 'governance', { status: 'completed', progress: 100 });

      // Citation validation
      if (taskId && taskManager) taskManager.updateProgress(taskId, 65 + round * 5, '正在校验引文...', 'citation-check');
      ...
      if (hasCitations) {
        ...
        if (citationReport.fabricatedCitations.length > 0) {
          pendingFixes = { instruction: 'Fix fabricated citations; use approved citation pool only.', round, citationReport: { fabricatedCitations: citationReport.fabricatedCitations.map(f => f.citeKey), unverifiedCitations: [], suggestedReplacements: [] } };
          section.status = 'needs_fix'; section.version++; round++;
          if (taskId && taskManager) taskManager.updateProgress(taskId, 65 + round * 5, `引文校验失败，进入第${round + 1}轮重试`);
          continue;
        }
      }
      if (taskId && taskManager) taskManager.updatePhase(taskId, 'citation-check', { status: 'completed', progress: 100 });
      ...
      if (!auditResult.passed) {
        pendingFixes = {
          instruction: `Fix ${auditResult.stats.critical} critical issues.`, round,
          auditReport: { criticals: auditResult.findings.filter(f => f.severity === 'critical').map(f => ({ dimension: f.dimension, severity: f.severity, finding: f.description, location: f.location })), warnings: [], infos: [] },
        };
        section.status = 'needs_fix'; section.version++; round++;
        if (taskId && taskManager) taskManager.updateProgress(taskId, 75 + round * 5, `审计未通过，进入第${round + 1}轮重试`);
        continue;
      }
      ...
      if (!skipAntiAI) {
        ...
        if (!aiOk) {
          ...
          section.status = 'needs_fix'; section.version++; round++;
          if (taskId && taskManager) taskManager.updateProgress(taskId, 85 + round * 3, `AI 痕迹过高，进入第${round + 1}轮重试`);
          continue;
        }
      }
      ...
      // Integrity check
      if (!skipAntiAI) {
        if (taskId && taskManager) taskManager.updateProgress(taskId, 90 + round * 3, '正在执行完整性校验...', 'integrity');
        const integrity = verifyIntegrity(normalizeResult.normalizedDraft, draft, bibleEntries.map(e => ({ id: e.id, category: e.category, key: e.key, value: e.value, immutable: e.immutable })));
        if (!integrity.passed) {
          pendingFixes = { instruction: 'Rewriter modified protected content. Restore and re-audit.', round, violations: integrity.violations.map(v => ({ type: v.type, expected: v.expected, actual: v.actual, location: v.location ?? '' })) };
          section.status = 'needs_fix'; section.version++; round++;
          if (taskId && taskManager) taskManager.updateProgress(taskId, 90 + round * 3, `完整性校验失败，进入第${round + 1}轮重试`);
          continue;
        }
      }
      ...
      const finalState = resolveStateAfterRound({
        round, maxRounds, auditOk: true, crossValOk: true, aiOk: true, integrityOk: true, aiChangedSignificantly: false,
      });
  ...
  async writeAllSections(paperId: string, outline: PaperOutline, options: WriteSectionOptions = {}): Promise<Section[]> {
  ...
  async resumeSection(paperId: string, outline: PaperOutline, sectionId: string, options: WriteSectionOptions = {}): Promise<{ section: Section; state: SectionWriteState }> {
  ...
  async getStatus(paperId: string): Promise<{ run: PipelineRun | undefined; sections: Array<{ id: string; status: string; version: number }> }> {
  ...
  private async initRun(paperId: string, phase: PipelinePhase, step: string): Promise<void> {
    ...
    const run: PipelineRun = existing ?? {
      id: `run-${paperId}`, paperId, currentPhase: phase, currentStep: step,
      status: 'running', currentStage: 0, artifactVersion: 1, createdAt: new Date(), updatedAt: new Date(),
    };
    run.currentPhase = phase; run.currentStep = step; run.updatedAt = new Date();
    this.currentRun = run;
