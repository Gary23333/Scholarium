...
export class FileSystemStorage implements PipelineStorage {
  ...
  async loadOutline(): Promise<PaperOutline | undefined> {
    try {
      return JSON.parse(await readFile(this.path('paper_outline.json'), 'utf-8'));
    } catch (e) {
      logger.warn('Failed to load outline', String(e));
      return undefined;
    }
  }
  ...
  async saveBlueprint(blueprint: SectionBlueprint, sectionId: string): Promise<void> {
    ...
    await writeFile(
      this.path('blueprints', `${sectionId}_blueprint.json`),
  ...
  async loadBlueprint(sectionId: string): Promise<SectionBlueprint | undefined> {
    try {
      return JSON.parse(await readFile(this.path('blueprints', `${sectionId}_blueprint.json`), 'utf-8'));
    } catch (e) {
      logger.warn(`Failed to load blueprint ${sectionId}`, String(e));
      return undefined;
    }
  }
  ...
  async loadSection(id: string): Promise<Section | undefined> {
    try {
      return JSON.parse(await readFile(this.path('sections', `${id}.json`), 'utf-8'));
    } catch (e) {
      logger.warn(`Failed to load section ${id}`, String(e));
      return undefined;
    }
  }
  ...
  async loadPipelineRun(_paperId: string): Promise<PipelineRun | undefined> {
    try {
      return JSON.parse(await readFile(this.path('pipeline_run.json'), 'utf-8'));
    } catch (e) {
      logger.warn('Failed to load pipeline run', String(e));
      return undefined;
    }
  }
}
...
export class InMemoryStorage implements PipelineStorage {
  ...
  async saveSection(s: Section): Promise<void> {
    this.sections.set(s.id, { ...s });
  }
  async loadSection(id: string): Promise<Section | undefined> {
    return this.sections.get(id);
  }
  async savePipelineRun(r: PipelineRun): Promise<void> {
    this.runs.set(r.paperId, { ...r });
  }
  async loadPipelineRun(paperId: string): Promise<PipelineRun | undefined> {
    return this.runs.get(paperId);
  }
  async saveOutline(o: PaperOutline): Promise<void> {
    this.outlines.set('default', { ...o });
  }
  async loadOutline(): Promise<PaperOutline | undefined> {
    return this.outlines.get('default');
  }
  async saveBlueprint(b: SectionBlueprint, sectionId: string): Promise<void> {
    this.blueprints.set(sectionId, { ...b });
  }
  async loadBlueprint(sectionId: string): Promise<SectionBlueprint | undefined> {
    return this.blueprints.get(sectionId);
  }
}
