...
export interface DomainReviewerInput { paperContent: string; paperTitle: string; reviewerConfig: any; }
...
export class DomainReviewerAgent extends BaseAgent<DomainReviewerInput, ReviewReport> {
  ...
  protected async realExecute(input: DomainReviewerInput): Promise<ReviewReport> {
    ...
    try {
      const content = await this.router.complete('domainReviewer', systemPrompt, `标题: ${input.paperTitle}\n\n${input.paperContent.slice(0, 4000)}`, { temperature: 0.2, maxTokens: 2048 });
      const parsed = JSON.parse(content.replace(/```json\n?/g, '').replace(/```\n?/g, '').trim());
      return this.buildReport(parsed, input);
    } catch (e) { logger.warn('DomainReviewer LLM failed', String(e)); return this.mockExecute(input); }
  }
  ...
  protected async mockExecute(input: DomainReviewerInput): Promise<ReviewReport> {
    return this.buildReport({
      ...
      findings: [
        { id: randomUUID(), dimension: 'literature', severity: 'minor', description: '缺少2024-2025年关键文献', suggestion: '补充最新的相关研究，特别是顶会和顶刊论文' },
      ],
      verdict: 'minor_revision', summary: '文献综述基本完整，但需要补充最新研究并加深理论对话。',
    }, input);
  }

  private buildReport(data: any, input: DomainReviewerInput): ReviewReport {
    return {
      reviewerId: randomUUID(), reviewerRole: 'domain', reviewerName: input.reviewerConfig?.name ?? 'R2',
      ...
      verdict: data.verdict ?? 'minor_revision', confidence: 0.8,
