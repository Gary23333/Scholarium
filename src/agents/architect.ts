...
Output ONLY valid JSON:
{
  ...
  "estimatedWords": number
    ...
    if (sectionId === '1') {
      purposeGuide =
        '段落目的分布建议：背景介绍由宽泛到具体（background为主）、前人研究综述（background）、研究不足与动机（motivation）、本文贡献与论文结构（transition）。避免开篇直接进入研究问题。';
    } else if (sectionId.startsWith('3')) {
      purposeGuide =
        '段落目的分布建议：问题形式化定义（method）、方法总述（method）、各子模块详述（method）、理论分析（discussion）。不同方法需分为不同段落/小节。';
    } else if (sectionId.startsWith('4')) {
      purposeGuide =
        '段落目的分布建议：实验目的回顾（transition）、总体结果概述（result）、各数据集结果（result）、与以往结果对比（discussion）、消融实验（result）、不足分析（discussion）。图和表需构建有效。';
    } else if (sectionId.startsWith('5')) {
      purposeGuide =
        '段落目的分布建议：全文工作总结（result）、核心贡献（discussion）、方法论局限（discussion）、未来展望（discussion）。必须承认无法解决的问题。';
    }
...
- transition: connect sections, outline structure, summarize`;
    ...
    const content = await this.router.complete('architect', systemPrompt, userPrompt, {
      maxTokens: 8192,
      timeout: 600000,
    });
  ...
  protected async mockExecute(input: ArchitectInput): Promise<SectionBlueprint> {
    ...
    if (section.id.includes('method')) {
      purposes.push({
        purpose: 'method',
        sentence: 'Formally, let $X$ denote the input space and $Y$ the output space.',
      });
      purposes.push({
        purpose: 'method',
        sentence: 'Our model optimizes the objective function via gradient descent.',
      });
    }
    if (section.id.includes('experiment')) {
      purposes.push({
        purpose: 'result',
        sentence: 'The proposed method achieves competitive accuracy on the test set.',
      });
      purposes.push({
        purpose: 'discussion',
        sentence: 'Ablation studies validate the contribution of each component.',
      });
    }
    return {
