...
export interface PerspectiveReviewerInput { paperContent: string; paperTitle: string; reviewerConfig: any; }
...
export class PerspectiveReviewerAgent extends BaseAgent<PerspectiveReviewerInput, ReviewReport> {
  ...
  protected async realExecute(input: PerspectiveReviewerInput): Promise<ReviewReport> {
    ...
    try {
      const content = await this.router.complete('perspectiveReviewer', systemPrompt, `标题: ${input.paperTitle}\n\n${input.paperContent.slice(0, 4000)}`, { temperature: 0.3, maxTokens: 2048 });
      const parsed = JSON.parse(content.replace(/```json\n?/g, '').replace(/```\n?/g, '').trim());
      return this.buildReport(parsed, input);
    } catch (e) { logger.warn('PerspectiveReviewer LLM failed', String(e)); return this.mockExecute(input); }
  }
  ...
  protected async mockExecute(input: PerspectiveReviewerInput): Promise<ReviewReport> {
    return this.buildReport({
      ...
      findings: [
        { id: randomUUID(), dimension: 'practical', severity: 'suggestion', description: '可以考虑从社会学或经济学视角补充分析', suggestion: '探讨与其他学科的交叉点' },
      ],
      verdict: 'minor_revision', summary: '论文具有一定的实践意义，但跨学科视角和更广泛的影响讨论需要加强。',
    }, input);
  }

  private buildReport(data: any, input: PerspectiveReviewerInput): ReviewReport {
    return {
      reviewerId: randomUUID(), reviewerRole: 'perspective', reviewerName: input.reviewerConfig?.name ?? 'R3',
      ...
      verdict: data.verdict ?? 'minor_revision', confidence: 0.75,
