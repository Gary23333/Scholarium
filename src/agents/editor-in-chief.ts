...
export interface EICInput { paperContent: string; paperTitle: string; field: string; reviewerConfig: any; }
...
export class EditorInChiefAgent extends BaseAgent<EICInput, ReviewReport> {
  ...
  protected async realExecute(input: EICInput): Promise<ReviewReport> {
    ...
    try {
      const content = await this.router.complete('editorInChief', systemPrompt, `标题: ${input.paperTitle}\n\n${input.paperContent.slice(0, 4000)}`, { temperature: 0.3, maxTokens: 2048 });
      const parsed = JSON.parse(content.replace(/```json\n?/g, '').replace(/```\n?/g, '').trim());
      return this.buildReport(parsed, input);
    } catch (e) { logger.warn('EditorInChief LLM failed', String(e)); return this.mockExecute(input); }
  }
  ...
  protected async mockExecute(input: EICInput): Promise<ReviewReport> {
    return this.buildReport({
      ...
      findings: [
        { id: randomUUID(), dimension: 'originality', severity: 'minor', description: '创新点表述不够突出', suggestion: '在引言中更明确地列出核心贡献' },
      ],
  ...
  private buildReport(data: any, input: EICInput): ReviewReport {
    return {
      reviewerId: randomUUID(), reviewerRole: 'eic', reviewerName: input.reviewerConfig?.name ?? 'EIC',
      ...
      verdict: data.verdict ?? 'major_revision', confidence: 0.8,
