...
export interface DevilsAdvocateInput { paperContent: string; paperTitle: string; reviewerConfig: any; }
...
export class DevilsAdvocateAgent extends BaseAgent<DevilsAdvocateInput, DevilsAdvocateReport> {
  ...
  protected async realExecute(input: DevilsAdvocateInput): Promise<DevilsAdvocateReport> {
    ...
    try {
      const content = await this.router.complete('devilsAdvocate', systemPrompt, `标题: ${input.paperTitle}\n\n${input.paperContent.slice(0, 4000)}`, { temperature: 0.4, maxTokens: 2048 });
      const parsed = JSON.parse(content.replace(/```json\n?/g, '').replace(/```\n?/g, '').trim());
      return this.buildReport(parsed, input);
    } catch (e) { logger.warn('DevilsAdvocate LLM failed', String(e)); return this.mockExecute(input); }
  }

  protected async mockExecute(input: DevilsAdvocateInput): Promise<DevilsAdvocateReport> {
    return this.buildReport({
      strongestCounterArgument: '论文的核心论点建立在问卷调查数据之上，但问卷的信效度验证不充分，且样本可能存在自选择偏差，这使得结论的外部效度受到质疑。',
      ...
      findings: [
        { id: randomUUID(), dimension: 'logic', severity: 'major', description: '核心论证存在因果推断跳跃', suggestion: '明确区分相关性和因果性，或补充因果推断方法' },
        { id: randomUUID(), dimension: 'evidence', severity: 'minor', description: '部分关键声称缺乏充分证据支撑', suggestion: '为每个关键声称提供引用或数据支撑' },
      ],
      verdict: 'major_revision', confidence: 0.7,
  ...
  private buildReport(data: any, input: DevilsAdvocateInput): DevilsAdvocateReport {
    return {
      reviewerId: randomUUID(), reviewerRole: 'da', reviewerName: '魔鬼代言人',
      expertise: '逻辑分析与论证评估',
      scores: {}, strengths: [], weaknesses: [],
      findings: (data.findings ?? []).map((f: any) => ({ ...f, id: f.id ?? randomUUID() })),
      verdict: data.verdict ?? 'major_revision', confidence: data.confidence ?? 0.7,
