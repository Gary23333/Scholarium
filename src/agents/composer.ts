...
import type { AgentOptions, BibleEntry, ContextPackage, FixInstructions, OutlineSection, PaperOutline } from '../types/index.ts';
...
export class ComposerAgent extends BaseAgent<ComposerInput, ContextPackage> {
  ...
  protected async realExecute(input: ComposerInput): Promise<ContextPackage> { return this.assemble(input); }
  protected async mockExecute(input: ComposerInput): Promise<ContextPackage> { return this.assemble(input); }
  ...
  private assemble(input: ComposerInput): ContextPackage {
    ...
    const previousSections = outline.sections.slice(0, currentIndex).map((s) => ({ id: s.id, title: s.title, summary: s.coreArgument }));
    return {
      outline,
