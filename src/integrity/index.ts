...
export function verifyIntegrity(
  original: string,
  rewritten: string,
  bible: BibleEntryStub[],
): ProtectedContentIntegrityReport {
  ...
  // Check Bible entries
  const protectedEntries = bible.filter((e) =>
    ['data', 'formulas', 'citations', 'variables', 'terminology'].includes(e.category),
  );
  for (const entry of protectedEntries) {
    totalChecked++;
    if (original.includes(entry.value) && rewritten.includes(entry.value)) {
      unchanged++;
  ...
  for (const key of origCites) {
    totalChecked++;
    if (!newCites.includes(key)) {
      removed++;
      violations.push({ type: 'citation', expected: `\\cite{${key}}`, actual: '(missing)' });
    } else {
      unchanged++;
    }
  }
  ...
  if (origEqs !== newEqs) {
    totalChecked++;
    violations.push({ type: 'formula', expected: `${origEqs} equations`, actual: `${newEqs} equations` });
  }

  return {
    passed: violations.length === 0,
    violations,
    stats: { totalChecked, unchanged, modified: 0, removed, added: 0 },
  };
}
